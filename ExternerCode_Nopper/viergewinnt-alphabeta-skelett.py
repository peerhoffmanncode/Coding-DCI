...
def max(restTiefe, alpha, beta):
    ...
    for y in range(7):
        if feld[0][y] == u:
            zug(y, 2)
            wert = min(restTiefe - 1, alpha, beta)
            if wert > alpha:
                alpha = wert
            zugRueckgaengig(y)
            if alpha >= beta:
...
def min(restTiefe, alpha, beta):
    ...
    for y in range(7):
        if feld[0][y] == u:
            zug(y, 0)
            wert = max(restTiefe - 1, alpha, beta)
            if wert < beta:
                beta = wert
            zugRueckgaengig(y)
            if beta <= alpha:
...
def minWo():
    ...
    for y in range(7):
        if feld[0][y] == u:
            zug(y, 0)
            wert = max(maximaleTiefe, alpha, beta)
            if wert < beta:
                beta = wert
                minimumY = y
                print(minimumY)
